...
function request_data(query::String)
    ...
    if r.status != 200
        ...
        elseif r.status == 500
            error("Internal Server Error: The database is not available at the time of the request.")
        elseif r.status == 503
            error("Service Unavailable: The server is currently unable to handle the request due to a temporary overloading or maintenance of the server.")
        else
