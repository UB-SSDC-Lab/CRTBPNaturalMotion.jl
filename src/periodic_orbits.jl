...
function TypeAPeriodicOrbit(
    ...
    return TypeAPeriodicOrbit(SA[rx, rz, vy], jacobi, stability, P, S, N_cross, mu, TU, DU)
end
"""
    TypeAPeriodicOrbit(
...
function GeneralPeriodicOrbit(
    ...
    # Compute orbit arc length
    S = propagate_return_arc_length(
        x0, (0.0, P), mu; solver=ode_solver, reltol=ode_reltol, abstol=ode_abstol
    )
...
function propagate_from_initial_conditions(
    ...
    if τ1 == 0.0
        ...
        return propagate_return_final_state(
            x0, tspan, orbit.mu, PT; solver=solver, reltol=reltol, abstol=abstol
        )
    end
end
...
function get_state_and_monodromy_matrix(
    ...
    x0 = propagate_from_initial_conditions(
        orbit, τ1, PT; solver=solver, reltol=reltol, abstol=abstol
    )
    M = propagate_return_final_stm(
        x0, (0.0, orbit.P), orbit.mu; solver=solver, reltol=reltol, abstol=abstol
    )
    return x0, M
end
...
function get_full_orbit(
    orbit::AbstractPeriodicOrbit; solver=Vern9(), reltol=1e-14, abstol=1e-14
)
...
function Base.show(io::IO, ::MIME"text/plain", orbit::TypeAPeriodicOrbit)
...
function Base.show(io::IO, ::MIME"text/plain", orbit::GeneralPeriodicOrbit)
    compact = get(io, :compact, false)
    if compact
