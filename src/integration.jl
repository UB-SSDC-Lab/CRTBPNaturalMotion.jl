...
@inline function propagate_return_all_states(
    ...
    return solve(
        ODEProblem{false,SciMLBase.FullSpecialize}(natural_crtbp_eom, x0, tspan, (mu,)),
        solver;
        abstol=abstol,
        reltol=reltol,
    )
...
@inline function propagate_return_all_states(
    x0::SVector{6,TX}, tspan::Tuple{TT,TT}, mu; solver=Vern9(), reltol=1e-14, abstol=1e-14
) where {TX,TT}
    return propagate_return_all_states(
        x0, tspan, mu, Time; solver=solver, reltol=reltol, abstol=abstol
    )
end
...
@inline function propagate_return_all_states(
    ...
    return solve(
        ODEProblem{false,SciMLBase.FullSpecialize}(
            natural_crtbp_eom_with_independant_arclen, x0, sspan, (mu,)
        ),
        solver;
        abstol=abstol,
        reltol=reltol,
...
@inline function propagate_return_all_states(
    ...
    ode_sol = solve(
        ODEProblem{false,SciMLBase.FullSpecialize}(
            natural_crtbp_eom, x0, (tsteps[0], tsteps[end]), (mu,)
        ),
        solver;
        tstops=tsteps,
        abstol=abstol,
...
@inline function propagate_return_all_states(
    x0::SVector{6,TX},
    ssteps::Union{StepRangeLen,AbstractVector},
    mu,
    ::Type{ArcLength};
    solver=Vern9(),
    reltol=1e-14,
    ...
    ode_sol = solve(
        ODEProblem{false,SciMLBase.FullSpecialize}(
            natural_crtbp_eom_with_independant_arclen, x0, (ssteps[1], ssteps[end]), (mu,)
        ),
        solver;
        tstops=ssteps,
        abstol=abstol,
...
@inline function propagate_return_all_states(
    ...
    return propagate_return_all_states(
        x0, tspan, mu, term_cond, Time; solver=solver, reltol=reltol, abstol=abstol
    )
end
...
@inline function propagate_return_arc_length(
    ...
    xfs = solve(
        ODEProblem{false,SciMLBase.FullSpecialize}(
            natural_crtbp_eom_with_arclen, x0s, tspan, (mu,)
        ),
        solver;
        abstol=abstol,
        reltol=reltol,
...
@inline function propagate_return_time(
    x0::SVector{6,TX}, sspan::Tuple{TT,TT}, mu; solver=Vern9(), reltol=1e-14, abstol=1e-14
) where {TX,TT}
    ...
    xfs = solve(
        ODEProblem{false,SciMLBase.FullSpecialize}(
            natural_crtbp_and_time_eom_with_independant_arclen, x0t, sspan, (mu,)
        ),
        solver;
        abstol=abstol,
        reltol=reltol,
...
@inline function propagate_return_final_stm(
    ...
    z_stop = solve(
        ODEProblem{false,SciMLBase.FullSpecialize}(
            natural_crtbp_eom_with_stm, z_start, tspan, (mu,)
        ),
        solver;
        abstol=abstol,
        reltol=reltol,
